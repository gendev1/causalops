...
import { validateAgentEnvironment } from '@repo/shared/env';
import { IndexCreationRequestSchema, type IndexCreationRequest } from './elastic-index-schema';
...
export class ElasticIndexInitializer {
  ...
  private createElasticClient(): Client {
    if ('ELASTIC_CLOUD_ID' in this.env) {
      ...
      return new Client({
        cloud: {
          id: this.env.ELASTIC_CLOUD_ID
        },
        auth: {
          apiKey: this.env.ELASTIC_API_KEY
        }
      });
    } else {
      ...
      return new Client({
        ...
        auth: {
          username: this.env.ELASTIC_USERNAME,
          password: this.env.ELASTIC_PASSWORD
        }
      });
    }
  }
  ...
  async initializeIndex(options: IndexInitOptions = {}): Promise<void> {
    const indexName = options.indexName || this.env.ELASTIC_INDEX;
